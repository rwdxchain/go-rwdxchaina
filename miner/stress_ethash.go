...
func main() {
	...
	ethash.MakeDataset(1, filepath.Join(os.Getenv("HOME"), ".rwdhash"))

	// Create an Rwdhash network based off of the Ropsten config
...
// makeGenesis creates a custom Rwdhash genesis block based on some pre-defined
// faucet accounts.
...
func makeMiner(genesis *core.Genesis, nodes []string) (*node.Node, error) {
	...
	// Start the node and configure a full Rwdxchain node on it
