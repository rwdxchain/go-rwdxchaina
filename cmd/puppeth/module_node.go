...
{{end}}
RUN \
  echo 'grwd --cache 512 init /genesis.json' > grwd.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.rwdxchain/keystore/ && cp /signer.json /root/.rwdxchain/keystore/' >> grwd.sh && \{{end}}
	echo $'exec grwd --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> grwd.sh

ENTRYPOINT ["/bin/sh", "grwd.sh"]
`
